The Premium of Proximity: Modeling the Impact of Amenities on HDB Resale Prices

Problem Statement
The Conflict: 
Homebuyers face a "sandwich" problem: they want convenience (MRT) and priority for their children (Primary Schools), but these often come with a massive “proximity premium.”
The Gap: 
While general HDB prices are known, it is difficult for buyers to quantify exactly how much extra they are paying for being <1km vs. >1km away from a specific amenity.

Target Audience
Homebuyers: To help them decide if a premium for top primary schools is worth it.
Real Estate Agents: To provide data-backed justifications for asking prices.


Documentation of files submitted

1. Technical Report

* The Premium of HDB Proximity - Shiyun.docx



2. Jupyter Notebook (run in Colab) –

* HDB Resale flat 2015 to 2025 Python – GeoAPI
* HDB Resale flat 2015 to 2025 Python - Analytics



3. Presentation Deck –

* PDF:
  Capstone Presentation Final - Shi Yun DABPT03
* Powerpoint:
  Capstone presentation Final - Shi Yun DABPT03



4. Original Dataset (needed to run Python Colab notebook)

* Resale Flat Prices (Based on Registration Date), From Jan 2015 to Dec 2016
* Resale flat prices based on registration date from Jan-2017 onwards
* Generalinformationofschools
* Train Station Codes and Chinese Names
* shopping\_mall\_coordinates
* ConsumerPriceIndexCPI2024AsBaseYearMonthly



5. Interactive Dashboard (Power BI)

* Power BI interactive visualisation
